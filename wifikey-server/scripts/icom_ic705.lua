-- ICOM IC-705 CI-V スクリプト
-- シリアル設定: 19200bps, 1stop, no parity (IC-705 メニューのデフォルト値)
--
-- メニュー推奨設定:
...
--
-- ATU:
--   IC-705 は内蔵 ATU を持たないため、外部 ATU (AH-705 等) を使用します。
--   ctl:assert_atu() 用の GPIO ピンを外部 ATU のスタートトリガーに接続してください。
--   IC-705 の最大出力は 10W のため、ATU チューニング時は 5W (set_power(50)) を使用します。
--
...
-- CI-V アドレス (IC-705 デフォルト 0xA4; メニューで変更可)
local CIV_ADDR  = 0xA4
local CTRL_ADDR = 0xE0  -- コントローラアドレス (PC 側)

-- モード一覧 (循環切替用)
-- IC-705 は DV (D-STAR) も対応するが CW 運用では不要のため省略
...
-- 周波数: リトルエンディアン 5バイト BCD (ICOM 形式)
...
-- 例: 255 (100%) = [0x02, 0x55], 128 (50%) = [0x01, 0x28]
...
-- パワー設定 (0〜100%)
-- IC-705 最大出力は 10W。100% = 10W、50% = 5W。
...
rig.actions = {
    start_atu = {
        ...
        fn = function(self, ctl)
            ...
            -- IC-705 最大 10W のため 50% = 5W でチューニング
            log_info("[ATU] 2/6 Setting CW 5W...")
            self:set_mode("CW")
            self:set_power(50)
            sleep_ms(500)
