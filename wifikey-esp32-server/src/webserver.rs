//! Configuration web server for AP mode (Server variant)
//!
//! Provides a simple web interface for configuring WiFi profiles.
//! This is the server variant with KEY_OUTPUT instead of KEY_INPUT.
...
/// HTML template for the configuration page (Server variant)
    ...
    <script>
        ...
        async function loadGpio() {
            try {
                ...
                document.getElementById('gpioKey').value = gpio.key_output;
                ...
                document.getElementById('gpioInfo').innerHTML =
                    `<p style="color:#888;margin-bottom:10px">Current: KeyOut=GPIO${gpio.key_output}, Btn=GPIO${gpio.button}, LED=GPIO${gpio.led}</p>`;
            } catch (e) {
        ...
        document.getElementById('gpioForm').onsubmit = async (e) => {
            ...
            const gpio = {
                key_output: parseInt(document.getElementById('gpioKey').value),
...
impl ConfigWebServer {
    ...
    pub fn start(config_manager: Arc<Mutex<ConfigManager>>) -> Result<Self> {
        ...
        // WiFi scan (returns empty - scanning in AP mode is complex)
        server.fn_handler::<anyhow::Error, _>(
            "/api/scan",
            esp_idf_svc::http::Method::Get,
            |req| {
...
/// Convert GPIO config to JSON (server variant with key_output)
fn gpio_to_json(gpio: &GpioConfig) -> String {
    format!(
        r#"{{"key_output":{},"button":{},"led":{}}}"#,
        gpio.key_output, gpio.button, gpio.led
    )
}

/// Parse GPIO config from JSON (server variant)
fn parse_gpio_json(data: &[u8]) -> Option<GpioConfig> {
    ...
    Some(GpioConfig {
        key_output: extract_num("key_output")?,
