//! Serial command interface for configuration (Server variant)
//!
...
fn help_text() -> String {
    format!(
        "WifiKey ESP32 Server Commands:\r\n\
         ...
         AT+GPIO=<key_out>,<btn>,<led> - Set GPIO pins (DANGER!)\r\n\
         AT+GPIO=DEFAULT - Reset GPIO to defaults\r\n\
         AT+RESTART  - Restart device\r\n\
         AT+INFO     - Show device info\r\n\
...
fn add_profile(args: &str, config_manager: &Arc<Mutex<ConfigManager>>) -> String {
    // Parse: ssid,wifipass,server_name,server_pass
    let parts: Vec<&str> = args.splitn(4, ',').collect();

    if parts.len() < 3 {
        return format!("Usage: AT+ADD=<ssid>,<wifipass>,<server_name>,<server_pass>\r\n{ERROR}");
    }
...
fn set_gpio(args: &str, config_manager: &Arc<Mutex<ConfigManager>>) -> String {
    ...
    // Parse: key_out,btn,led
    let parts: Vec<&str> = args.split(',').collect();
    if parts.len() != 3 {
        return format!(
            "Usage: AT+GPIO=<key_out>,<btn>,<led>\r\n\
    ...
    let key_output = match parse_pin(parts[0]) {
        Ok(v) => v,
    ...
    match config_manager.lock().unwrap().save_gpio_config(&config) {
        Ok(_) => format!(
            "GPIO set: key_out={key_output}, btn={button}, led={led}{warning}{OK}"
        ),
        Err(e) => format!("Failed: {e:?}\r\n{ERROR}"),
    }
}
