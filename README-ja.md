...
```
  ESP32 (Client)              MQTT Broker              PC (Server)
       ...
       │                          │                          │
       ╔══════════════════════════╧══════════════════════════╗
       ║  UDP Hole Punching → KCP 暗号化セッション             ║
       ║  (LAN内アドレスを優先使用)                            ║
       ╚═════════════════════════════════════════════════════╝
```
...
**v0.3.0** より、wifikey-server は **Luaスクリプト** による汎用リグ制御 (CAT - Computer Aided Transceiver) をサポートしています。サーバーのソースコードを変更することなく、CAT対応の任意の無線機を制御できます。

### 仕組み

サーバーにサンドボックス化された **Lua 5.4** VM ([mlua](https://crates.io/crates/mlua)) を組み込んでいます。Luaスクリプトは、シリアルポート設定、CATプロトコルコマンド、およびオプションのカスタムアクションを無線機モデルごとに定義します。
...
同一LAN内の場合、**mDNS** (`_wifikey2._udp.local.`) がMQTT/STUNと並行してゼロコンフィグ探索に使用されます。

- サーバー: `mdns-sd` クレートでサービス公開
- クライアント: `esp-idf-svc::mdns` でサービス探索
- LAN内接続は常にWAN接続より優先
...
- **暗号化**: ChaCha20-Poly1305認証付き暗号化
- **ATU制御**: アンテナチューナー起動機能
- **Lua CATスクリプティング**: Luaスクリプトによる拡張可能なリグ制御
