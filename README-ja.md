...
### PC不要の構成 (wifikey --features server)
...
`--features server` を指定すると、同じ `wifikey` クレートがスタンドアロンサーバーとして動作します。ESP32がGPIO出力でフォトカプラを駆動し、PCなしで無線機をキーイングします。
...
```
  ESP32 (Client)              MQTT Broker              Server (PC/ESP32)
       │                          │                          │
       │  [経路A: mDNS — 同一LAN内のみ、経路Bと並行実行]
       ├─ mDNS問い合わせ (マルチキャスト) ──────────────────► │ (公開中)
       │◄─ IP:ポート ──────────────────────────────────────── ┤
       │   → 見つかれば: 直接KCP接続、経路Bは不要             │
       │                          │                          │
       │  [経路B: MQTT + STUN/IPv6 — LAN・WAN対応]           │
       ├──SUBSCRIBE───────────────►                          │
       │                          ◄───────────SUBSCRIBE──────┤
       │  ┌─────────────┐        │        ┌─────────────┐  │
       │  │IPv6プローブ  │        │        │IPv6プローブ  │  │
       │  │またはSTUN(v4)│        │        │またはSTUN(v4)│  │
       │  └─────────────┘        │        └─────────────┘  │
       │                          │                          │
...
- **NAT traversal**: MQTT + STUNによる接続確立 (IPv4) または直接接続 (IPv6)
- **デュアルスタック (IPv4/IPv6)**: 完全なデュアルスタック実装。IPv6利用可能時は優先 (NAT不要、低遅延)。Windowsプライバシー拡張アドレス・SLAACタイミング・ステートフルファイアウォール越えホールパンチングを透過的に処理。
...
**解決策**: STUN (Session Traversal Utilities for NAT) でグローバルアドレスを取得し、UDPホールパンチングでNATを越える。グローバルIPv6アドレスが利用可能な場合はSTUNをスキップ — IPv6ではNATが不要で、双方向にプローブパケットを送り合うことでステートフルファイアウォールの状態エントリを作成する。

対応NAT種別 (IPv4):
- **Full Cone NAT**: 完全対応
- **Restricted Cone NAT**: 対応
- **Port Restricted Cone NAT**: 対応
- **Symmetric NAT (クライアント側のみ)**: 対応 — サーバー側がFull Cone / Restricted NATであれば、クライアントが先に送信してNATマッピングを作成し、サーバーが同じアドレスで応答するため通信可能
- **Symmetric NAT (双方)**: 非対応 (TURNリレーが必要)

多くの家庭用ルーターやモバイルキャリアはCone系NATです。IPv6環境ではNATを完全に回避でき、ステートフルファイアウォール越えもホールパンチングで対処します。
...
| IPv6 WAN | ✓ 直接接続 + ステートフルFW越えホールパンチング (NAT不要・STUN不要) |
| 家庭用ルーター (Cone NAT) | ✓ STUN + UDPホールパンチング |
| モバイルキャリア (IPv4 CGNAT) | ✓ STUN + UDPホールパンチング |
| モバイルキャリア (IPv6) | ✓ IPv6直接接続 — NAT不要・低遅延 |
| Symmetric NAT (クライアント側) + Full Cone (サーバー側) | ✓ 対応 (クライアント起点マッピング) |
| Symmetric NAT (双方、IPv4のみ) | ✗ 非対応 |

> **日本の環境**: IPoE光回線 (v6プラス / DS-Lite / MAP-E) および主要モバイルキャリア (docomo / au / SoftBank) はすべてグローバルIPv6を提供しており、IPv6経路でほぼすべての実用シナリオをカバーできます。
...
ESP32クライアントはリトライごとに次の順序でサーバーを探索します:

1. **mDNS** — LAN内マルチキャスト。同一ネットワーク内なら約5秒で解決
2. **MQTT/STUN** — WANシグナリング。双方が最良アドレスを公開（`esp_netif_get_ip6_global()` でIPv6が利用可能ならIPv6優先、それ以外はSTUN経由のIPv4）し、到達可能な最良アドレスを選択

グローバルIPv6アドレスが検出された場合はIPv6を優先します（NAT不要で低遅延）。IPv6での接続が認証段階で失敗した場合、クライアントは自動的にIPv4のみで再試行します。接続成功後はデュアルスタック動作に戻ります。
...
rig_script = "yaesu_ft891.lua"  # Lua CATスクリプト名
```
...
3. 後からWeb設定でNVSプロファイルを保存した場合、そちらが優先されます
...
`wifikey --features server` を使用すると、PCなしでリモートキーイングが可能です。
...
```
AT+GPIO     # GPIO設定表示 (KEY_GPIO, BUTTON, LED)
...
```powershell
# M5Atom Lite にサーバーファームウェアをビルド・フラッシュ
.\flash.ps1 -Server
...
│   └── src/
│       ├── main.rs               #   エントリーポイント (WiFi, mDNS, パドル/キーイング, LED)
│       ├── config.rs             #   プロファイル保存 (NVS)
│       ├── keyer.rs              #   GPIOキーイング出力 (server featureのみ)
│       ├── webserver.rs          #   APモード設定UI
│       ├── wifi.rs               #   WiFi接続
│       └── serial_cmd.rs         #   ATコマンドハンドラ
│
...
```powershell
# 基本的な使い方 (クライアント, M5Atom Lite, COMポート自動検出, debugビルド)
.\flash.ps1

# サーバーファームウェアをビルド・フラッシュ
.\flash.ps1 -Server
...
| `-Server` | スイッチ | オフ | サーバーファームウェアをビルド (`--features server`) |
...
| `cargo make esp-server-build` | ESP32サーバービルド (debug, `--features server`) |
| `cargo make esp-server-build-release` | ESP32サーバービルド (release, `--features server`) |
| `cargo make esp-monitor` | シリアルモニタ |
| `cargo make esp-erase` | ESP32フラッシュ消去 |
| `cargo make esp-clippy` | ESP32クライアント clippy |
| `cargo make esp-server-clippy` | ESP32サーバー clippy (`--features server`) |
