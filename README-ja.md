...
- **NAT traversal**: MQTT + STUNによる接続確立 (IPv4) またはホールパンチング直接接続 (IPv6)
- **IPv6対応**: デュアルスタックIPv6/IPv4。IPv6利用可能時は優先 (NAT不要、低遅延)
...
**解決策**: STUN (Session Traversal Utilities for NAT) でグローバルアドレスを取得し、UDPホールパンチングでNATを越える。グローバルIPv6アドレスが利用可能な場合はSTUNをスキップ — IPv6ではNATが不要で、ステートフルファイヤーウォール越えのホールパンチングのみ必要。
...
多くの家庭用ルーターやモバイルキャリアはCone系NATのため、STUNで接続可能です。IPv6環境ではNATを完全に回避できます。
...
| IPv6 WAN | ✓ 直接ホールパンチング (NAT不要、STUN不要) |
| 家庭用ルーター (Cone NAT) | ✓ STUN + UDPホールパンチング |
| モバイルキャリア (多くの場合) | ✓ STUN + UDPホールパンチング |
| Symmetric NAT | ✗ 非対応 |
...
| `wifikey-server` | 0.3.3 | デスクトップGUIアプリ (**Tauri 2.x**) |
